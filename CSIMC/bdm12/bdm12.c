/* operate the Kevin Ross BDM12 Pod from Linux.
...
static char deftty[] = "/dev/ttyS0";	/* default tty */
#define	PODSPEED	B57600		/* pod speed */
...
typedef enum {LT_RAM, LT_EE, LT_FLASH} LoadType;
...
usage(char *pname)
{
    ...
    fprintf(stderr, "Purpose: command line interface to BDM12 Pod.\n");
    fprintf(stderr, "Set SW1=Off SW2=On SW3=Off SW4=Off\n");
    fprintf(stderr, "Options:\n");
    fprintf(stderr, " -c n   : instead of polling CTS, count down from <n>\n");
...
static int
a_qs (const void *p1, const void *p2)
{
...
static int
n_qs (const void *p1, const void *p2)
{
...
showCmds(void)
{
	...
	printf("P  <filename>\t\tload the given S-record file into EEPROM (or RAM)  *\n");
...
printRegs ()
{
	...
	writePodByte (0x63); readPodByte(&h); readPodByte(&l);
	printf ("PC:%02x%02x ", h, l);
	tryMapFile ((((int)h)<<8) | l);

	writePodByte (0x64); readPodByte(&h); readPodByte(&l);
	printf ("D:%02x%02x ", h, l);

	writePodByte (0x65); readPodByte(&h); readPodByte(&l);
	printf ("X:%02x%02x ", h, l);

	writePodByte (0x66); readPodByte(&h); readPodByte(&l);
	printf ("Y:%02x%02x ", h, l);

	writePodByte (0x67); readPodByte(&h); readPodByte(&l);
 ...
 */
static int
writeFlashW (int addr, int data)
{
	int back;

	if (addr & 1) {
	    if (writeFlashB (addr, data >> 8) == 0 &&
		writeFlashB (addr+1, data & 0xff) == 0)
		    return (0);
	} else {
	...
	/* oh oh */
	printf ("\nFlash write failed at 0x%x: wrote 0x%x read back 0x%x\n",
							    addr, data, back);
	return (-1);
}
...
static void
fillRAM (int a0, int a1, int data)
{
	int a;
	int n;
...
static void
loadSFile (char name[], LoadType lt, int verify)
{
	struct timeval tv0, tv1;
	char buf[1024], *bp;
	FILE *fp;
 ...
 */
static int
load1S (int addr, char buf[], int nleft, LoadType lt, int verify)
{
	int data;
	...
	if (!(addr & 1) && nleft >= 2) {
	    ...
	    switch (lt) {
	    case LT_RAM: writeWord (addr, data); break;
	    case LT_EE: writeEEW (addr, data); break;
	    case LT_FLASH:
	    ...
	    switch (lt) {
	    case LT_RAM: writeByte (addr, data); break;
	    case LT_EE: writeEEB (addr, data); break;
	    case LT_FLASH:
...
static void
writePodBuf (int addr, char str[], int n)
{
	while (n-- > 0)
	    writeByte (addr++, *str++);
}
...
writePodByte (byte b)
{
	...
	/* queue and wait for CTS toggle, or just forget it */
	ioctl (podfd, TIOCGICOUNT, &sic);
	ncts = sic.cts;
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: bdm12.c,v $ $Date: 2003/04/15 20:46:47 $ $Revision: 1.1.1.1 $ $Name:  $"};
