...
nextToken(void)
{
	...
	case INWHITE:
	    ...
	    case '{': tok = BLKSRT; break;
	    case '}': tok = BLKEND; break;
	    case ',': tok = COMMA; break;
	    case '(': tok = LPAREN; break;
	    case ')': tok = RPAREN; break;
	    case '?': tok = CHIF; break;
	    case ':': tok = CHELSE; break;
	    case '~': tok = COMP; break;
	    case ';': tok = SEMI; break;
	...
	case TWINOP:
	    ...
	    case '=':
		switch (hist) {
		case '+': tok = ADDOP; break;
		case '-': tok = SUBOP; break;
		case '*': tok = MULTOP; break;
		case '/': tok = DIVOP; break;
		case '%': tok = MODOP; break;
		case '^': tok = XOROP; break;
		case '|': tok = OROP; break;
		case '&': tok = ANDOP; break;
		case '<': tok = LE; break;
		case '>': tok = GE; break;
		case '!': tok = NE; break;
		case '=': tok = EQ; break;
	    ...
	    case '&':
		if (hist == '&') tok = LAND; else goto chkhist;
		break;

	    case '|':
		if (hist == '|') tok = LOR; else goto chkhist;
		break;

	    case '+':
		if (hist == '+') tok = INC; else goto chkhist;
		break;

	    case '-':
		if (hist == '-') tok = DEC; else goto chkhist;
		break;

	    case '>':
		if (hist == '>') state = GG; else goto chkhist;
		break;

	    case '<':
		if (hist == '<') state = LL; else goto chkhist;
		break;

	    case '*':
		if (hist == '/') state = CCOMMENT; else goto chkhist;
		break;

	    case '/':
		if (hist == '/') state = EOLCOMMENT; else goto chkhist;
		break;
 ...
 * @(#) $RCSfile: lex.c,v $ $Date: 2003/04/15 20:46:49 $ $Revision: 1.1.1.1 $ $Name:  $
 */
