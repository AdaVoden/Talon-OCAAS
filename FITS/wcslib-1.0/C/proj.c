...
*   (*fwd) and reverse (*rev) transformation.  A separate function (*set)
*   initializes intermediate values formed from the projection parameters and
...
*      coeset coefwd coerev   CEA: conic equal area
...
*      prj.flag
*         This flag must be set to zero whenever any of prj.p[10] or prj.r0
*         are set or changed.  This signals the initialization routines to
...
*
*   Usage of the prj.p[] array as it applies to each projection is described
*   in the prologue to each trio of projection routines.
*
*   Argument checking
*   -----------------
*   Forward routines:
...
*   $Id: proj.c,v 1.1.1.1 2003/04/15 20:46:59 lostairman Exp $
*===========================================================================*/
...

{
   ...
   z =  sind(theta) - 1.0;
   *x =  prj->r0*(cosd(theta)*sind(phi) + prj->p[1]*z);
   *y = -prj->r0*(cosd(theta)*cosd(phi) + prj->p[2]*z);
...
{
   double a, b, c, d, pxy, sth, sth1, sth2, x0, xp, y0, yp;
   ...
   x0 = x*prj->w[0];
   y0 = y*prj->w[0];
   pxy = 2.0*(prj->p[1]*x0 - prj->p[2]*y0);
   a = prj->w[3];
   b = -(pxy + prj->w[2]);
   c = (x0*x0 + y0*y0) + pxy + prj->w[4];
   d = b*b - 2.0*a*c;
   ...
   if (sth > 1.0) sth = (sth1<sth2) ? sth1 : sth2;
   if (sth > 1.0 || sth < -1.0) {
      return 2;
   }

   /* Compute native coordinates. */
   xp = -y0 - prj->p[2]*(sth-1.0);
   yp =  x0 - prj->p[1]*(sth-1.0);
   if (xp == 0.0 && yp == 0.0) {
      *phi = 0.0;
   } else {
      *phi   = atan2d(yp,xp);
...
*
*   Given:
*      prj->p[1:2] Latitudes of the standard parallels, theta1 and theta2,
*                  in degrees.
*
*   Given and/or returned:
*      prj->r0     r0; reset to 180/pi if 0.
*      prj->w[0]   C  = sin((theta1 + theta2)/2)
*      prj->w[1]   1/C
*      prj->w[2]   Y0 = 0
*      prj->w[3]   (theta1 + theta2)/2
*      prj->w[4]   r0*cos((theta2 - theta1)/2)
*      prj->w[5]   1/(r0*cos((theta2 - theta1)/2))
*      prj->w[6]   cot((theta1 + theta2)/2)
*===========================================================================*/
...

{
   ...
   prj->w[0] = sind((prj->p[2] + prj->p[1])/2.0);
   if (prj->w[0] <= 0.0) {
      return 1;
   }
   ...
   prj->w[1] = 1.0/prj->w[0];
   prj->w[2] = 0.0;
   prj->w[3] = (prj->p[2] + prj->p[1])/2.0;

   prj->w[4] = prj->r0*cosd((prj->p[2] - prj->p[1])/2.0);
   if (prj->w[4] == 0.0) {
      return 1;
   }

   prj->w[5] = 1.0/prj->w[4];
   prj->w[6] = 1.0/tand(prj->w[3]);
...

{
   ...
   a = prj->w[0]*phi;
   r = prj->w[4]*(prj->w[6] - tand(theta-prj->w[3]));
...

{
   ...
   *phi   = a*prj->w[1];
   *theta = prj->w[3] + atand(prj->w[6] - r*prj->w[5]);
...
*   Given and/or returned:
*      prj->r0     r0; reset to 180/pi if 0.
*      prj->w[0]   C = r0*sin(sigma)*sin(delta)/delta, where
*                    sigma = (theta1+theta2)/2
*                    delta = (theta2-theta1)/2
*      prj->w[1]   1/C
*      prj->w[2]   Y0 = sigma - 90 +alpha*cot(alpha)*cot(sigma)
*      prj->w[3]   Y0 + 90
*===========================================================================*/
...
{
   double thedif, thesum;

   if (prj->r0 == 0.0) prj->r0 = R2D;

   thesum = (prj->p[2] + prj->p[1])/2.0;
   thedif = (prj->p[2] - prj->p[1])/2.0;

   if (thedif == 0.0) {
      prj->w[0] = prj->r0*sind(thesum)*D2R;
   } else {
      prj->w[0] = prj->r0*sind(thesum)*sind(thedif)/thedif;
   }
   ...
   prj->w[3] = thesum + prj->r0*cosd(thedif)*cosd(thesum)/prj->w[0];
   prj->w[2] = prj->w[3] - 90.0;
...
/*============================================================================
*   CEA: conic equal area projection.
*
*   Given:
*      prj->p[1:2] Latitudes of the standard parallels, theta1 and theta2,
*                  in degrees.
...
*      prj->w[1]   1/C
*      prj->w[2]   Y0 = r0*sqrt(1 + sin(theta1)*sin(theta2) - 2C)/C
*      prj->w[3]   (1 + sin(theta1)*sin(theta2))*(r0/C)**2
*      prj->w[4]   2*r0**2/C
*      prj->w[5]   C/(2*r0**2)
*===========================================================================*/
...

{
   ...
   prj->w[0] = (sind(prj->p[1]) + sind(prj->p[2]))/2.0;
   if (prj->w[0] <= 0.0) {
      return 1;
   }
   ...
   w = 1.0 + sind(prj->p[1])*sind(prj->p[2]);
   prj->w[2] = prj->r0*sqrt(w - 2.0*prj->w[0])/prj->w[0];
   prj->w[3] = prj->r0/prj->w[0], prj->w[3] *= w*prj->w[3];

   prj->w[4] = 2.0*prj->r0*prj->r0/prj->w[0];
   prj->w[5] = 1.0/prj->w[4];
   prj->w[6] = prj->r0*sqrt(w + prj->w[0]+prj->w[0])/prj->w[0];
...

{
   ...
   if (theta == -90.0) {
      ...
      r = sqrt(prj->w[3] - prj->w[4]*sind(theta));
   }
...

{
   ...
   *phi = a*prj->w[1];
   if (fabs(r-prj->w[6]) < tol) {
      *theta = -90.0;
   } else {
      w = (prj->w[3] - r*r)*prj->w[5];
      if (fabs(w) > 1.0) {
         if (fabs(w-1.0) < tol) {
            *theta = 90.0;
...
*      prj->w[1]   1/C
*      prj->w[2]   Y0 = 0
*      prj->w[3]   (r0*cos(theta1)/C)/tan(tau1)**C
*      prj->w[4]   C/(r0*cos(theta1))*tan(tau1)**C
*===========================================================================*/
...
{
   double cos1, cos2, tan1, tan2;

   if (prj->r0 == 0.0) prj->r0 = R2D;

   tan1 = tand((90.0 - prj->p[1])/2.0);
   cos1 = cosd(prj->p[1]);

   if (prj->p[1] == prj->p[2]) {
      prj->w[0] = sind(prj->p[1]);
   } else {
      tan2 = tand((90.0 - prj->p[2])/2.0);
      cos2 = cosd(prj->p[2]);
...
*   Given and/or returned:
*      prj->r0     r0; reset to 180/pi if 0.
*      prj->w[0]   Y0 = r0*cot(theta1) + theta1
*===========================================================================*/
...

{
   if (prj->r0 == 0.0) {
      prj->r0 = R2D;
      prj->w[0] = prj->r0*cosd(prj->p[1])/sind(prj->p[1]) + prj->p[1];
      prj->w[1] = 1.0;
   } else {
      prj->w[0] = prj->r0*(cosd(prj->p[1])/sind(prj->p[1]) + prj->p[1]*D2R);
      prj->w[1] = prj->r0*D2R;
   }
...

{
   ...
   float a, a2, a2b2, a4, b, b2, b4, ca2, cb2, x0, xf, y0, yf;
...
{
   int   face;
   double chi, costhe, eta, l, m, n, psi, rho, x0, xf, xi, y0, yf;
   const double tol = 1.0e-12;
   ...
   if (xi == 0.0 && eta == 0.0) {
      ...
      psi = eta/xi;
      chi = 1.0 + psi*psi;
      xf  = -sqrt((1.0-rho)/(1.0-1.0/sqrt(1.0+chi)));
      ...
      psi = xi/eta;
      chi = 1.0 + psi*psi;
      yf  = -sqrt((1.0-rho)/(1.0-1.0/sqrt(1.0+chi)));
...
{
   int   direct, face;
   double chi, l, m, n, psi, rho, xf, yf, w;
   const double tol = 1.0e-12;
   ...
   if (direct) {
      if (xf == 0.0) {
         ...
         chi = 1.0 + psi*psi;
         rho = 1.0 - xf*xf*(1.0 - 1.0/sqrt(1.0 + chi));
      }
   } else {
      if (yf == 0.0) {
         psi = 0.0;
         ...
         psi = sind(w)/(cosd(w) - SQRT2INV);
         chi = 1.0 + psi*psi;
         rho = 1.0 - yf*yf*(1.0 - 1.0/sqrt(1.0 + chi));
      }
   }
   ...
   if (fabs(rho) >= 1.0) {
      ...
      rho = copysign(1.0,rho);
      w = 0.0;
   } else {
      w = sqrt((1.0-rho*rho)/chi);
   }
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: proj.c,v $ $Date: 2003/04/15 20:46:59 $ $Revision: 1.1.1.1 $ $Name:  $"};
