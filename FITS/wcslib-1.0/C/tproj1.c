...
*
*   $Id: tproj1.c,v 1.1.1.1 2003/04/15 20:47:00 lostairman Exp $
*---------------------------------------------------------------------------*/
...
/* call ieee_handler ("set", "common", SIGFPE_ABORT); */
   ...
   prj.p[1] = 45.0;
   prj.p[2] = 75.0;
   projex("COO", coofwd, coorev, &prj, 90, -85, tol);
...

{
   int    err, lat, lng;
   register int j;
   double lng1, lng2, lat1, lat2, x, y;
   ...
   for (lat = north; lat >= south; lat--) {
      ...
      for (lng = -180; lng <= 180; lng++) {
         ...
         if (fabs(lat2-lat1) > tol) {
            printf("%3s: lng1 =%20.15lf  lat1 =%20.15lf\n",
               pcode, lng1, lat1);
            printf("        x =%20.15lf     y =%20.15lf\n", x, y);
            printf("     lng2 =%20.15lf  lat2 =%20.15lf\n", lng2, lat2);
         } else if (abs(lat) != 90) {
            if (fabs(lng2-lng1) > tol) {
               if (fabs(fabs(lng2-lng1)-360.0) > tol) {
                  printf("%3s: lng1 =%20.15lf  lat1 =%20.15lf\n",
                     pcode, lng1, lat1);
                  printf("        x =%20.15lf     y =%20.15lf\n", x, y);
                  printf("     lng2 =%20.15lf  lat2 =%20.15lf\n", lng2, lat2);
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: tproj1.c,v $ $Date: 2003/04/15 20:47:00 $ $Revision: 1.1.1.1 $ $Name:  $"};
