...
static int currentpm;			/* index into pminfo currently up */
...
nextStep()
{
	if (XmToggleButtonGetState(cyclic_w)) {
	    ...
	    currentpm = (cyc_backwards ? (--currentpm + npminfo)
	    			       : (++currentpm)) % npminfo;
	}
}
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: blink.c,v $ $Date: 2003/04/15 20:47:01 $ $Revision: 1.1.1.1 $ $Name:  $"};
