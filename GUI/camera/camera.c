...
// STO 2002-10-07 Switch to using median instead of mean bias averaging (see also calimage.c)
#define MEDIAN_BIAS
...
/*
 * STO 2002-10-07
 * This version computes a median bias rather than a mean
 *
 */
static int nextBias ()
{
	...
	int n, mid;
	CamPixel ** biasList = (CamPixel **) expinfo.fpmem;
	if(!biasList) {
...
#ifdef MEDIAN_BIAS
		if(expinfo.state == BIAS_EXP) {
			CamPixel ** cp = (CamPixel **) expinfo.fpmem;
			int i;
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: camera.c,v $ $Date: 2003/04/15 20:47:01 $ $Revision: 1.1.1.1 $ $Name:  $"};
