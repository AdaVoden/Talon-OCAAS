...
setWindow()
{
	...
	if (XmToggleButtonGetState(auto_w)) {
		...
		if (lo < 0)
		    lo = 0;
		hi = (int)state.stats.mean + 2*state.stats.sd;
		if (hi >= NCAMPIX)
		    hi = NCAMPIX - 1;
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: winhist.c,v $ $Date: 2003/04/15 20:47:03 $ $Revision: 1.1.1.1 $ $Name:  $"};
