...
char *av[];
{
	...
	n = 0;
	XtSetArg (args[n], XmNiconName, myclass); n++;
...
fillSI()
{
	...
	switch (telstatshmp->telstate) {
	case TS_STOPPED: strcpy (buf1, "STOPPED "); break;
	case TS_SLEWING: strcpy (buf1, "SLEWING "); break;
	case TS_HUNTING: strcpy (buf1, "HUNTING "); break;
	case TS_TRACKING:strcpy (buf1, "TRACKING"); break;
	case TS_HOMING:  strcpy (buf1, "HOMING  "); break;
	case TS_LIMITING:strcpy (buf1, "LIMITS  "); break;
	default:         strcpy (buf1, "<UNKNOWN>"); break;
	}
	if (tap->ZENFLIP)
	    strcat (buf1, " ZENFLIP");
	if (tap->GERMEQ) {
	...
	if (telstatshmp->domestate != DS_ABSENT) {
	    ...
	    case DS_STOPPED:  strcpy (buf3, "STOPPED  "); break;
	    case DS_ROTATING: strcpy (buf3, "ROTATING "); break;
	    case DS_HOMING:   strcpy (buf3, "HOMING "); break;
	    default:          strcpy (buf3, "<UNKNOWN>"); break;
	    }
	...
	if (telstatshmp->shutterstate != SH_ABSENT) {
	    ...
	    case SH_ABSENT:  strcpy (buf1, "<NONE>"); break;
	    case SH_IDLE:    strcpy (buf1, "IDLE"); break;
	    case SH_OPENING: strcpy (buf1, "OPENING"); break;
	    case SH_CLOSING: strcpy (buf1, "CLOSING"); break;
	    case SH_OPEN:    strcpy (buf1, "OPEN"); break;
	    case SH_CLOSED:  strcpy (buf1, "CLOSED"); break;
	    default:         strcpy (buf1, "<UNKNOWN>"); break;
	    }
	...
	case CAM_IDLE: strcpy (buf1, "IDLE     "); break;
	case CAM_EXPO: strcpy (buf1, "EXPOSING "); break;
	case CAM_READ: strcpy (buf1, "READING  "); break;
	default:       strcpy (buf1, "<UNKNOWN>"); break;
	}
...
makeMain ()
{
	...
	XtSetArg (args[n], XmNset, want_sections); n++;
	...
	XtSetArg (args[n], XmNset, want_queue); n++;
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: shm.c,v $ $Date: 2003/04/15 20:47:04 $ $Revision: 1.1.1.1 $ $Name:  $"};
