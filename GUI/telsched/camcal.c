...
{
	switch (style) {
	case CT_BIAS:    cp->new = CT_BIAS; cp->data = CD_NONE; break;
	case CT_THERMAL: cp->new = CT_THERMAL; cp->data = CD_NONE; break;
	case CT_FLAT:    cp->new = CT_FLAT; cp->data = CD_NONE; break;
	default: fprintf (stderr, "Bogus style: %d\n", style); exit (1);
	}
}
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: camcal.c,v $ $Date: 2003/04/15 20:47:04 $ $Revision: 1.1.1.1 $ $Name:  $"};
