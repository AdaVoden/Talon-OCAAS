...
int *nopp;
{
	...
	for (i = 1; i < ndup; i++)
	    bop[i].scan.ccdcalib = bop[0].scan.ccdcalib;
    ...
    if (kwp->sawit) {
        ...
        f = atof(kwp->value);
        for(i = 0; i < ndup; i++) {
            bop[i].scan.ccdcalib.new = CT_FOCUSPOS;
    ...
    if (kwp->sawit) {
        ...
        f = atof(kwp->value);
        for(i = 0; i < ndup; i++) {
            bop[i].scan.ccdcalib.new = CT_FOCUSOFF;
            bop[i].scan.extval1 = f;
        }
    }
    ...
    if (kwp->sawit) {
        ...
        for(i = 0; i < ndup; i++) {
            bop[i].scan.ccdcalib.new = CT_AUTOFOCUS;
            bop[i].scan.extval1 = f;
        }
    }
    ...
    if (kwp->sawit) {
    	...
    	for (i = 0; i < ndup; i++) {
            ...
            bop[i].scan.ccdcalib.new = CT_FIXEDALTAZ;
    	}
    }
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: obs.c,v $ $Date: 2003/04/15 20:47:04 $ $Revision: 1.1.1.1 $ $Name:  $"};
