...
Obs *op;
{
	...
	if (op->scan.ccdcalib.data == CD_RAW)
	    ...
	    fprintf (fp, " C");
	else if (op->scan.ccdcalib.new == CT_BIAS)
	    fprintf (fp, " B");
	else if (op->scan.ccdcalib.new == CT_THERMAL)
	    fprintf (fp, " T");
	else if (op->scan.ccdcalib.new == CT_FLAT)
	    fprintf (fp, " F");
	else
	    fprintf (fp, " ?");
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: print.c,v $ $Date: 2003/04/15 20:47:04 $ $Revision: 1.1.1.1 $ $Name:  $"};
