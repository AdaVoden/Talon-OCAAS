...
#define	NSL		11	/* rows in the scrolled list */
...
init_now()
{
	 ...
	 */
	t = time(NULL);
	now.n_mjd = mjd_day (25567.5 + 1.0 + t/SPD);
	now.n_epoch = EOD;
...
XtPointer call;
{
	...
	if (i == 0) {
	    ...
	    print_sls (&now, workop, nworkop, imdir, filename);
	    XtFree (filename);
	}
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: telsched.c,v $ $Date: 2003/04/15 20:47:05 $ $Revision: 1.1.1.1 $ $Name:  $"};
