...
#define	LISTROWS	6	/* rows in list */
...
static char avhost[] = "charts.aavso.org";	/* host */
static char avcgi[] = "/cgi-bin/xephemstar.pl";	/* aavso's cgi-bin script */
static char default_str[] = "default";		/* knows to use nice interval */
static char latest_str[] = "latest";		/* knows to use latest */

static void av_create P_((void));
static void av_help_cb P_((Widget w, XtPointer client, XtPointer call));
static void av_close_cb P_((Widget w, XtPointer client, XtPointer call));
static void av_print_cb P_((Widget w, XtPointer client, XtPointer call));
static void av_list_cb P_((Widget w, XtPointer client, XtPointer call));
static void av_go_cb P_((Widget w, XtPointer client, XtPointer call));
static void av_name_cb P_((Widget w, XtPointer client, XtPointer call));
static void mkPrompt P_((Widget parent, char *prompt, Widget *text_wp,
    char *name));
static void mkTop P_((Widget rc_w));
static FILE * openDB P_((void));
static int fetchpm P_((void));
static void cleanLine P_((char *line));
static void fetchAndShow P_((void));
static void av_print P_((void));
...
Widget rc_w;
{
	...
	set_xmstring (w, XmNlabelString, "Name and date range:");
	XtManageChild (w);
	...
	XtManageChild (w);
	set_xmstring (w, XmNlabelString, "Near last Sky View pick:"); 
...
XtPointer call;
{
	if (curve_pm)
	    ...
	    xe_msg ("No light curve to print", 1);
}
...
av_print()
{
	...
	XPSPixmap (curve_pm, wid, hei, xe_cm, 0);
	XPSXEnd();
	XPSClose();
...
XtPointer call;
{
	...
	*ucp = '\0';
	...
	sprintf (buf, "Can not find '%s' in %s", nameuc, avdb);
	xe_msg (buf, 1);
}
...
fetchpm()
{
	static char fmt[] = "%s?%s?%s?%s HTTP/1.0\r\nUser-Agent: xephem/%s\r\n\r\n";
	Display *dsp = XtDisplay(toplevel_w);
	int nrawgif;
	char buf[1024];
	char jd1[32], jd2[32];
	...
	/* query server */
	(void) sprintf (buf, fmt, avcgi, jd1, jd2, id, PATCHLEVEL);
	xe_msg (buf, 0);
	...
	if (!isgif) {
		...
		xe_msg (buf, 0);
	    xe_msg ("Error talking to AAVSO .. see File->Messages\n", 1);
	    close (fd);
	    stopd_down();
	    watch_cursor(0);
	    return (-1);
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: aavso.c,v $ $Date: 2003/04/15 20:47:05 $ $Revision: 1.1.1.1 $ $Name:  $"};
