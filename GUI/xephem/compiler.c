...
static int next_token P_((void));
static int chk_funcs P_((void));
static void skip_double P_((void));
static int compile P_((int prec));
static int execute P_((double *result));
static int parse_fieldname P_((char name[], int len));
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: compiler.c,v $ $Date: 2003/04/15 20:47:06 $ $Revision: 1.1.1.1 $ $Name:  $"};
