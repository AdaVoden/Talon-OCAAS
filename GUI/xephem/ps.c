...
typedef struct {
    Font fid;			/* Font ID */
    char *name;			/* malloced name of PS font to use */
...
#define	NPIXCACHE	128	/* XColors we cache for speed */
...
static PSFontInfo * find_psfont P_((Font fid));
...
static void x_fill_circle P_((Display *dsp, Drawable win, GC gc, int x, int y,
    int diam));
static void pixCache P_((XColor *xcp));
...
XPSClose()
{
	...
	fprintf (FP, "(%s) 306 %d cstr\n",
			    "(c) 1996-2000 Elwood Charles Downey", AROWY(2));
	fprintf (FP, "(%s) 306 %d cstr\n",
 ...
 * N.B. this also sets overimg which means all subsequent text will be drawn in
 *   white (unless wantcolor is on).
 */
void
XPSPixmap (pm, wid, hei, cm, bggc)
...
GC bggc;		/* background pixel to not draw, or 0 */
{
	...
	if (xpsc.state != XDRAWING || xpsc.win != pm)
	    return;
	...
	if (!xim) {
	    xe_msg ("Can not create image for printing", 1);
	    return;
	}
	...
	/* use a little cache to reduce using XQueryColor every time */
	pixCache (NULL);
 ...
 * N.B. we assume xe_cm is the colormap.
 */
static void
...
int dogray;
{
	...
	if (xpsc.wantcolor) {
		...
		/* basically gray */
		if (v > .8)
		    v = .8;		/* white doesn't show */
		else if (v < .5)
		    v = .5;		/* save some ink if black */
	    } else {
		/* rich color */
		s = 1.0;
	    }
...
Font fid;
{
	static Atom FAMILY, WEIGHT, SLANT, PIXELSZ;
	Display *dsp = XtDisplay(toplevel_w);
	int (*oldhandler)() = XSetErrorHandler (IgnoreError);
	char family[32], weight[32], slant[32];
	char psname[64];
	PSFontInfo *fi;
	XFontStruct *fsp;
	...
	/* gather atoms property into a cache */
	if (!FAMILY) {
	    FAMILY = XInternAtom (dsp, "FAMILY_NAME", 1);
	    WEIGHT = XInternAtom (dsp, "WEIGHT_NAME", 1);
	    SLANT = XInternAtom (dsp, "SLANT", 1);
	    PIXELSZ = XInternAtom (dsp, "PIXEL_SIZE", 1);
	}

	/* grow fi[] */
	psfi = (PSFontInfo *) XtRealloc ((void *)psfi,
						(npsfi+1)*sizeof(PSFontInfo));
	fi = &psfi[npsfi];

	/* build the PS name from various properties */
	fsp = XQueryFont (dsp, fid);
	if (!fsp) {
	    xe_msg ("No font info!", 1);
	    return (NULL);
	}

	if (!XGetFontProperty (fsp, FAMILY, &card32)) {
	    xe_msg ("No FAMILY font property", 1);
	    return (NULL);
	}
	name = XGetAtomName (dsp, card32);
	strcpy (family, name);
	if (islower(family[0]))
	    family[0] = toupper(family[0]);
	XtFree (name);

	if (!XGetFontProperty (fsp, WEIGHT, &card32)) {
	    xe_msg ("No WEIGHT font property", 1);
	    return (NULL);
	}
	name = XGetAtomName (dsp, card32);
	switch (name[0]) {
	...
	if (!XGetFontProperty (fsp, SLANT, &card32)) {
	    xe_msg ("No SLANT font property", 1);
	    return (NULL);
	}
	name = XGetAtomName (dsp, card32);
	switch (name[0]) {
	...
	if (!XGetFontProperty (fsp, PIXELSZ, &card32)) {
	    xe_msg ("No PIXEL_SIZE font property", 1);
	    return (NULL);
	}
	pixsz = card32;
	XtFree (name);
	...
	*/
	...
	sprintf (psname, "%s%s%s", family, slant, weight);
	fi->name = XtNewString (psname);
	fi->h = pixsz;
	fi->fid = fid;
	npsfi++;
...
create_print_w()
{
	...
	XtManageChild (rb_w);
	    ...
	    sr_reg (gray_w, NULL, printcategory, 1);

	    /* Grayscale inits both */
	    XmToggleButtonSetState(color_w, !XmToggleButtonGetState(gray_w), 0);
	...
	XtManageChild (f_w);
	    ...
	    defaultTextFN (filename_w, 1, getPrivateDir(), "xephem.ps");
...
/* look up xcp->pixel in a cache.
 * if not found, add if possible.
...
XColor *xcp;
{
	...
	if (!xpsc.wantcolor) {
	    double r = (double)xcp->red;
	    double g = (double)xcp->green;
	    double b = (double)xcp->blue;
	    int gray = (int)sqrt ((r*r + g*g + b*b)/3.0);
	    xcp->red = xcp->green = xcp->blue = gray;
	}

	/* add to cache in ascending order, if more room */
	if (nxc < NPIXCACHE) {
	    for (t = nxc-1; xc[t].pixel > xcp->pixel; --t)
		xc[t+1] = xc[t];
	    xc[t+1] = *xcp;
	    nxc++;
	}
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: ps.c,v $ $Date: 2003/04/15 20:47:13 $ $Revision: 1.1.1.1 $ $Name:  $"};
