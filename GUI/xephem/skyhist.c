...
#define	VERSIONN		3	/* version number */
...
static void svh_manage_cb P_((Widget w, XtPointer client, XtPointer call));
static void svh_add_cb P_((Widget w, XtPointer client, XtPointer call));
static void svh_del_cb P_((Widget w, XtPointer client, XtPointer call));
static void svh_rep_cb P_((Widget w, XtPointer client, XtPointer call));
static void svh_go_cb P_((Widget w, XtPointer client, XtPointer call));
static void svh_up_cb P_((Widget w, XtPointer client, XtPointer call));
static void svh_down_cb P_((Widget w, XtPointer client, XtPointer call));
static void svh_cascading_cb P_((Widget w, XtPointer client, XtPointer call));
static void svh_save_cb P_((Widget w, XtPointer client, XtPointer call));
static void svh_load_cb P_((Widget w, XtPointer client, XtPointer call));
static void svh_ltfc_cb P_((Widget w, XtPointer client, XtPointer call));
static void svh_help_cb P_((Widget w, XtPointer client, XtPointer call));
static void svh_close_cb P_((Widget w, XtPointer client, XtPointer call));
static void show_entries P_((void));
static void setup_1row P_((Widget rc_w, int i));
static void setup_label P_((Widget lbl_w, SvHistory *hp));
static FILE *openhist P_((char *buf, char *how));
static int loadhist P_((FILE *histfp, char *fn, char msg[]));
...
int i;
{
	...
	XtAddCallback (go_w, XmNactivateCallback, svh_go_cb, (XtPointer)i);
	...
	up_w = XmCreatePushButton (f_w, "Up", args, n);
	XtAddCallback (up_w, XmNactivateCallback, svh_up_cb, (XtPointer)i);
	...
	down_w = XmCreatePushButton (f_w, "Down", args, n);
	XtAddCallback (down_w, XmNactivateCallback, svh_down_cb, (XtPointer)i);
	...
	XtAddCallback (del_w, XmNactivateCallback, svh_del_cb, (XtPointer)i);
	...
	XtAddCallback (rep_w, XmNactivateCallback, svh_rep_cb, (XtPointer)i);
	...
	XtAddCallback (tf_w, XmNvalueChangedCallback, svh_ltfc_cb,(XtPointer)i);
...
XtPointer call;
{
	...
	for (i = 0; i < nhist; i++) {
		...
		XtAddCallback (pb,XmNactivateCallback, svh_go_cb, (XtPointer)i);
...
"Collects and Installs collections of Sky View settings",
	...
	hlp_dialog ("Sky View - history", msg, sizeof(msg)/sizeof(msg[0]));
}
...
XtPointer call;
{
	...
	if (confirm()) {
	    ...
	    query (toplevel_w, buf, "Yes -- delete", "No -- keep it", NULL,
							    del_i, NULL, NULL);
	} else
	    del_i();
}
...
XtPointer call;
{
	...
	if (confirm()) {
	    ...
	    query (toplevel_w, buf, "Yes -- replace", "No -- keep it", NULL,
							    rep_i, NULL, NULL);
	} else
	    rep_i();
}
...
XtPointer call;
{
	...
	for (i = 0; i < nhist; i++) {
		...
		hp->lbl_lst, hp->lbl_lss, hp->lbl_lds,
		hp->lbl_bst, hp->lbl_bss, hp->lbl_bds);
...
char *msg;
{
	...
	while (fgets(buf, sizeof(buf), histfp)) {
	    ...
	    "%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d",
		    ...
		    &hp.eyep, &hp.magscale, &hp.automag,
		    &hp.lbl_lst, &hp.lbl_lss, &hp.lbl_lds,
		    &hp.lbl_bst, &hp.lbl_bss, &hp.lbl_bds) != 24) {
		sprintf (msg, "%s:\nBogus switches line .. \n%s", fn, buf);
		goto err;
	    }
	    ...
	    if (!fgets(buf, sizeof(buf), histfp))
		goto err;
	    memset(hp.type_table, 0, sizeof(hp.type_table));
	    for (j = 0; buf[j] != '\0'; j++) 
		hp.type_table[j] = buf[j]=='1';

	    if (!fgets(buf, sizeof(buf), histfp))
		goto err;
	    memset(hp.fclass_table, 0, sizeof(hp.fclass_table));
	    for (j = 0; buf[j] != '\0'; j++) 
		hp.fclass_table[j] = buf[j]=='1';
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: skyhist.c,v $ $Date: 2003/04/15 20:47:16 $ $Revision: 1.1.1.1 $ $Name:  $"};
