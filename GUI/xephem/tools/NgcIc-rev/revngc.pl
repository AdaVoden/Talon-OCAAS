...
# Objects with type=7,8 are dropped since 
#      type=7 objects are written out immediately after being identified in 
#      the ID1,ID2 fields of their type<>7 originals
#      type=8 objects are in the IC.edb catalog
# if no magnitude is given, 19.99 is assigned by default.
# for the definitions of type= 1..10, see Steinecke's explan.htm file
# Submitted 22 Jun 2000 by Fridger Schrempp, t00fri@mail.desy.de

open (NGC, ">NGC.edb") || die "Can not create NGC.edb\n";
open (TMP, "|sort -k 3 -g >Tmp.edb")|| die "Can not create Tmp.edb\n";
open (IC8, ">IC8.edb") || die "Can not create IC8.edb\n";
...
while (<>) {
    ...
    # drop type=7,8 objects, since they are accounted for already! 
    # type=8 should better be evaluated in combination with IC catalog
    next if ($type =~/7/);
    if($type =~/8/){
      print IC8 $_ ;
      next;
