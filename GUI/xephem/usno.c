...
char msg[];	/* filled with error message if return -1 */
{
	...
	rov = raddeg (fov)/2;
	if (rov >= 7.5) {
	    strcpy (msg, "USNO FOV must be <= 15 degrees");
	    return (-1);
	}
...
addGS (StarArray *sap, FieldStar *fsp)
{
	...
	/* make up a name */
	sprintf (op->o_name, "USNO %06d", sap->used);
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: usno.c,v $ $Date: 2003/04/15 20:47:21 $ $Revision: 1.1.1.1 $ $Name:  $"};
