...
init_mount_cor()
{
        setConfigFile ("archive/config/telsched.cfg");
	readDVal ("PTGRAD", &ptgrad);
...
interp (double ha, double dec, double *ehap, double *edecp)
{
	...
	if (nfound >= 2) {
		...
		*edecp = closestrp->ddec;
		if (teled_trace)
		    tdlog ("Mesh:   closest point: %g %g %g %g\n",
				closestrp->ha, closestrp->dec, *ehap, *edecp);
	    } else {
		*ehap = 0.0;
		*edecp = 0.0;
		if (teled_trace)
		    tdlog ("Mesh:   no solution\n");
	    }
	}
}
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: mountcor.c,v $ $Date: 2003/04/15 20:47:56 $ $Revision: 1.1.1.1 $ $Name:  $"};
