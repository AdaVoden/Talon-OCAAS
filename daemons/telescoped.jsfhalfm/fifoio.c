 ...
 * wait up to telstatshmp->dt then timeout.
 */
void
chk_fifos()
{
	int dt = telstatshmp->dt;
	FifoInfo *fip;
	struct timeval tv;
	fd_set rfdset;
	int maxfdp1;
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: fifoio.c,v $ $Date: 2003/04/15 20:47:58 $ $Revision: 1.1.1.1 $ $Name:  $"};
