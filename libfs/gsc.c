 ...
 * GSCFetch(): return an array of FieldStars matching the given criteria.
 */
...
static char *syserrstr() { return (strerror(errno)); }
static FILE *fopenh (char *name, char *how) { return (fopen (name, how)); }
...
static int handleRequest P_((Request *qp, GSCArray *ap));
static int fetchRegion P_((GSCRegion *rp, Request *qp, GSCArray *ap));
static int inFOV P_((Request *qp, GSCEntry *ep));
static int magOK P_((Request *qp, GSCEntry *ep));
static int addOneStar P_((GSCArray *ap, GSCRegion *rp, GSCEntry *ep));
static int mymkdir P_((char *path));
static int myaccess P_((char *file));

static void gscPickRegion P_((double ra, double dec, GSCRegion *rp));
static int gscOpenRegion P_((GSCRegion *rp));
static int gscSimpleFits P_((FILE *fp));
static void gscOpenFile P_((GSCRegion *rp, char msg[]));
static int gscGetNextEntry P_((GSCRegion *rp, GSCEntry *ep));
static void gscCloseRegion P_((GSCRegion *rp));
static int cacheCreateRegion P_((GSCRegion *rp, GSCRegion *cp));

static int cacheOpenRegion P_((GSCRegion *rp, GSCRegion *cp));
static int cacheGetNextEntry P_((GSCRegion *rp, GSCEntry *ep));
static void cacheCloseRegion P_((GSCRegion *rp));
static void cacheAddEntry P_((GSCRegion *rp, GSCEntry *ep));
static void cacheBuildFilename P_((char fullpath[], GSCRegion *rp));
static void cacheBuildDir P_((char fullpath[], GSCRegion *rp));
static int cacheReadHeader P_((GSCRegion *rp));
static void cacheWriteHeader P_((GSCRegion *rp));
static void cacheReadEntry P_((GSCRegion *rp, GSCEntry *ep));
static void cacheWriteEntry P_((GSCRegion *rp, GSCEntry *ep));
...
char msg[];
{
	...
	if (chpath) {
	    ...
	    if (myaccess (fullpath) < 0) {
		(void) sprintf (msg, "%s: %s", chp, syserrstr());
		return (-1);
	    }
	}
...
/* create or add to a malloced array of FieldStar at *spp in the given region
 *   and return the new total count.
 * if spp == NULL we don't malloc anything but just do the side effects;
 * else *spp already has nspp FieldStar in it (it's ok if *spp == NULL).
...
FieldStar **spp;/* *spp will be a malloced array of FieldStar stars in region */
int nspp;	/* if spp: initial number of FieldStar already in *spp */
...
GSCEntry *ep;
{
	...
	if (ap->used >= ap->max) {
	    ...
	    ap->mem = (FieldStar *)newmem;
	    ap->max += NINC;
	}

	sp = (FieldStar *)&ap->mem[ap->used++];

	(void) sprintf (sp->name, "GSC %04d-%04d", rp->id, ep->id);
	sp->isstar = !ep->class;
	sp->ra = (float)ep->ra;
	sp->dec = (float)ep->dec;
	sp->mag = (float)ep->mag;
...
/* return 0 if we can open the file, else -1 */
static int
myaccess (file)
char *file;
{
...
GSCRegion *rp;
{
	...
	double raLrg;	/* fractions of large RA regions to target */
	double decLrg;	/* fractios of large Dec regions to target */
...
GSCRegion *rp;
{
	...
	if (gscSimpleFits (rp->fp) < 0) {
	    (void) sprintf (lmsg, "%s: not a SIMPLE FITS file", fn);
	    fclose (rp->fp);
	    rp->fp = NULL;
	    return (-1);
	}
	...
	for (n = 2, state = FINDNAXIS2; state != FOUNDTABLE; n++) {
		...
		(void) sprintf (lmsg, "%s: unexpected EOF", fn);
		fclose (rp->fp);
		rp->fp = NULL;
		return (-1);
	    }
...
GSCEntry *ep;
{
	...
	if (feof(rp->fp) || ferror(rp->fp)) {
	    fprintf (stderr, "%s: %d stars short\n", rp->file, rp->nrows);
	    return(-1);
	}
...
GSCRegion *cp;
{
	...
	if (!cp->fp) {
	    if (nocdrom)
		(void) sprintf (lmsg, "%s: %s", fullpath, syserrstr());
	    return (-1);
	}
	...
	if (rp->south != cp->south || rp->id != cp->id
	    ...
	    (void) sprintf (lmsg, "%s: header mismatch", fullpath);
...
GSCRegion *rp;
{
	...
	if (i == sizeof(hdr)) {
	    ...
	    cacheBuildFilename (fullpath, rp);
	    hdr[sizeof(hdr)-1] = '\0';
	    (void) sprintf (lmsg, "%s: no header: %s", fullpath, hdr);
	    return (-1);
	}
	...
	if (i != 6) {
	    char fullpath[128];
	    cacheBuildFilename (fullpath, rp);
	    (void) sprintf (lmsg, "%s: bad header: %s", fullpath, hdr);
	    return (-1);
	}
	...
	if (v > cacheversion) {
	    char fullpath[128];
	    cacheBuildFilename (fullpath, rp);
	    (void) sprintf (lmsg, "%s: unsupported version: %g", fullpath, v);
	    return (-1);
	}
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: gsc.c,v $ $Date: 2003/04/15 20:48:17 $ $Revision: 1.1.1.1 $ $Name:  $"};
