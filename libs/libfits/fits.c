...
char *errmsg;
{
	...
	nbytes = fip->sw * fip->sh * 2;	/* 2 bytes per pixel */
	fip->image = malloc (nbytes);
	if (!fip->image) {
	    sprintf (errmsg, "Could not malloc %d for pixels", nbytes);
	    resetFImage (fip);
	    return (-1);
	}
	 ...
	 */
	for (ntot = 0; ntot < nbytes; ntot += s) {
	    s = read (fd, fip->image + ntot, nbytes - ntot);
	    if (s <= 0) {
		if (s < 0)
		    strcpy (errmsg, strerror (errno));
		else
...
char errmsg[];
{
	...
	if (getIntFITS (fip, "BITPIX", &i) < 0 || i != 16) {
	    sprintf (errmsg, "BITPIX must be 16.");
	    goto err;
	}
	fip->bitpix = i;
