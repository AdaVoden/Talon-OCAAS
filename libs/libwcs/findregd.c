 ...
 * "distance" matching) and this new function is called with four extra
 * arguments, three of which correspond to config params TRYSTARS, MATCHDIST,
 * REJECTDIST.  The other, nparam, is the number of parameters in the
 * astrometric fit.  If nparam = 5, distance matching is attempted and if
 * successful a 5-parameter WCS fit is determined, as previously (except the
 * previous version attempted "triangle" based matching).  If nparam = 12, 20
 * or 26, a 5-parameter WCS fit is assumed to exist already in the input fits
 * header, and findRegistrationD uses position matching to (try to) firstly
 * get an updated WCS fit and then do a 12-parameter quadratic fit,
 * 20-parameter cubic fit, or fit using 26 parameters corresponding to those
 * used in the Digitized Sky Survey.
 *
...
#define NENLARGE    4    /* when doing higher order fits, max number of loops
...
/* Large resids get discarded during NOUTLL loops.  However, once the
 * NENLARGE/NOUTLL loops are finished, the fit is nevertheless accepted if
 * its median resid is below the max acceptable value given as an input
 * parameter.
 */
...
findRegistrationD (fip, ra0, dec0, rot0, psx0, psy0, sx, sy, ns, gr, gd, ng,
                   nparam, TRYSTARS, MATCHDIST, rhp, rp)
...
{
	int npair1, minpair, ienlarge, hiordok=0;
...
#define GNUPLOT_TRACE
	...
	/* 1st time we reach this point, function has operated similarly to
	 * old version except for different matching algorithm -
	 * have looked for 5-parameter WCS fit, not higher order fit,
	 * whatever input value of nparam
	 ...
	 * finish
	 * If we are at this point for the 2nd time, it means we found a
	 * higher order fit and came back to this part of the program to
	 * write gnuplot files; again, tidy up and finish
	 * But if nparam > 5 and we haven't searched for higher order fit,
	 * then we now do so
	 * Reason for this spaghetti programming (looping back to gnuplot
	 * lines of code) is that we want to write the gnuplot files
	 * immediately after finding the WCS fit, just in case the attempt to
	 * find a higher order fit fails (smx, smy, gmr, gmd will have been
	 * overwritten while seeking higher order fit, preventing WCS
	 * residuals from being written to /tmp/wcs.fit)
	 *
	...
	if (nparam == 5 || hiordok == 1) goto tidy;
	...
	for (ienlarge = 0; ienlarge < NENLARGE; ienlarge++) {
	  ...
	  if (npair < minpair) {hiordok = 0; goto out2;}
	  ...
	  for (j = 0; j < NOUTLL; j++) {
	    ...
	    int converged;
	    npair_g = npair;
	    /* conceivable that least squares routine gives error but that on
	     * an earlier pass through NHIORD loop, a solution was found.
	     * can fix this (by storing a[], b[], resid & fact that we have
	     * solution) but not bothering at present.
	     * if it happens, just lose the solution.
	     * similar comment applies to culling of high residuals not
	     * working as well as it did a previous time (either losing the
	     * solution, or ending up with a less good solution).
	     */
...
#endif
	    if (converged < 0) {hiordok = 0; goto out2;}
	    for (i = 0; i < npair; i++) residsort[i]=resid_g[i];
	    dmedian (residsort, npair, &rmed);
    ...
    out2:
	  ...
	  if (rmed > *rhp) hiordok = 0;
	  if (! hiordok) break;
	  if (ienlarge > 0 && npair <= npair1) break;
	  npair1 = npair;
