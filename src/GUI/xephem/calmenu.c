...
static void mm_nfmoon P_((double jd, double tzone, int m, int f, int nd));
...
Now *np;
{
	...
	/* proceed if f_ison() changed or on and it's a different day or tz */
	ison = f_ison();
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: calmenu.c,v $ $Date: 2003/04/15 20:47:06 $ $Revision: 1.1.1.1 $ $Name:  $"};
