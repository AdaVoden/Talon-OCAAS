...
static void c_create_shell P_((void));
...
static void c_popdown_cb P_((Widget w, XtPointer client, XtPointer call));
...
static int get_options P_((double *sp, double *mp, int *wp));
static void free_band P_((Band bp[], int nb));
static int init_bands P_((double sep, double mag, Band **bpp));
static int add_obj P_((Band *bp, Obj *op));
static int find_close P_((Band bp[], int nb, double maxsep, int wander,
    Pair **ppp));
static int add_pair P_((Obj *op1, Obj *op2, double sep, Pair **ppp,
    int *mp, int *np));
static int add_pair P_((Obj *op1, Obj *op2, double sep, Pair **ppp,
    int *mp, int *np));
static int dspl_pairs P_((Pair p[], int np, double sep));
...
c_create_shell ()
{
	...
	pd_w = XmCreatePulldownMenu (mb_w, "ControlPD", args, n);
	    ...
	    wtip (w, "Scan one time for close pairs");
	    XtManageChild (w);
	    ...
	    wtip (autoup_w, "When on, perform a new scan after each Main menu Update");
	...
	pd_w = XmCreatePulldownMenu (mb_w, "OptionsPD", args, n);
	    ...
	    wtip (wander_w, "When on, both objects in a pair can not be of type FIXED");
...
do_search()
{
	...
	if (get_options (&sep, &mag, &wander) < 0)
	    return;
	 ...
	 */
	np = find_close (bp, nb, sep, wander, &pp);
 ...
 */
static int
find_close (bp, nb, maxsep, wander, ppp)
...
c_create_flist_w()
{
	...
	defaultTextFN (tw, 1, getPrivateDir(), "closelist.txt");
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: closemenu.c,v $ $Date: 2003/04/15 20:47:06 $ $Revision: 1.1.1.1 $ $Name:  $"};
