...
msg_create_w()
{
	...
	txt_w = XmCreateScrolledText (msg_w, "Messages", args, n);
	XtManageChild (txt_w);
}
...
char *p;
{
	...
	if (!apmsg_w) {
	    ...
	    n = 0;
	    XtSetArg (args[n], XmNtitle, "xephem Message");  n++;
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: msgmenu.c,v $ $Date: 2003/04/15 20:47:12 $ $Revision: 1.1.1.1 $ $Name:  $"};
