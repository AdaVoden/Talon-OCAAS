...
Now *np;
{
	...
	t = (time_t)((mjd0 - 25567.5) * (3600.0*24.0) + 0.5);
	...
	if (!before) {
	    xe_msg ("No __STDC__ so no timezone info from system.", 0);
	    before = 1;
	}
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: time.c,v $ $Date: 2003/04/15 20:47:20 $ $Revision: 1.1.1.1 $ $Name:  $"};
