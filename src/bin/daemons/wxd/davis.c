/* device-depend support for Davis Weather Monitor II */
 ...
 */
int DUpdate(char *tty, WxStats *wp, double *tp, double *pp) {
  static int dfd = -1;
...
static void processDavis(unsigned char *rcv_buf, int barcal, WxStats *wp,
                         double *tp, double *pp) {
  double outside_temperature = *((short *)(rcv_buf + 2)) / 10.;
  double wind_speed = *((unsigned char *)(rcv_buf + 4));
  double wind_direction = *((short *)(rcv_buf + 5));
  double barometer = (*((short *)(rcv_buf + 7)) - barcal) / 1000.;
  double outside_humidity = *((unsigned char *)(rcv_buf + 10));
  double rain = *((short *)(rcv_buf + 11)) / 100.;
 ...
 */
static int rcv_crc(unsigned char *rcv_buf, int n) {
  unsigned short crc;
  int i;
