 ...
 */
int RWUpdate(char *tty, WxStats *wp, double *tp, double *pp) {
  static int fd = -1;
...
static int processRW(unsigned char *rcv_buf, int barcal, WxStats *wp,
  ...
  s = sscanf(rcv_buf, "D,%*[^,],%*[^,],%lf,%lf,%lf,%lf,%lf,%lf,%lf",
             &temperature, &humidity, &barometer, &wind_direction, &wind_speed,
             &hi_wind_speed, &rain);
  if (s != 7) {
 ...
 */
static int rcv_chksum(unsigned char *rcv_buf) {
  int sum;
  int c;
  int i;
 ...
 * return 0 if ok, else -1
 */
static int get_line(int fd, char rcv_buf[]) {
  int c, n;
