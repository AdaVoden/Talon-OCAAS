...
char *av[];
{
...

  {
    ...
    int homed = 1;
    if (homed && telstatshmp->minfo[TEL_HM].have)
      homed = telstatshmp->minfo[TEL_HM].ishomed;
    if (homed && telstatshmp->minfo[TEL_DM].have)
      homed = telstatshmp->minfo[TEL_DM].ishomed;
    if (homed && telstatshmp->minfo[TEL_RM].have)
      homed = telstatshmp->minfo[TEL_RM].ishomed;
    if (homed && telstatshmp->minfo[TEL_OM].have)
      homed = telstatshmp->minfo[TEL_OM].ishomed;
    if (homed && telstatshmp->minfo[TEL_IM].have)
      homed = telstatshmp->minfo[TEL_IM].ishomed;
    if (!homed) {
      char msg[] = "Telescope has not been fully homed. Cannot run schedule\n";
      tlog(NULL, msg);
      fifoWrite(Tel_Id, msg);
      return (-1);
    }
  }
 ...
 */
int addProgram(PrFunc p, int bkg) {
  int i;
...
#define NCCFG (sizeof(ccfg) / sizeof(ccfg[0]))
  static CfgEntry tcfg[] = {
      {"SUNDOWN", CFG_DBL, &SUNDOWN}, {"SETUP_TO", CFG_DBL, &SETUP_TO},
      {"STOWALT", CFG_DBL, &STOWALT}, {"STOWAZ", CFG_DBL, &STOWAZ},
      {"IGSUN", CFG_INT, &IGSUN},
  };
...
static void sigshutdown(int sig) {
...
static void unlockus() { unlock_running(progname, 0); }
