...
# $Header: /cvsroot/observatory/talon/talon/tools/web/cgi-lib.pl,v 1.1.1.1 2003/04/15 20:48:46 lostairman Exp $
#
# Copyright 1995 Afan Ottenheimer
# Added PrintVariablesText,
# Cleaned up output from PrintVariables
#
...
#Uncomment the following line for diagnostic 1
#  $post_string = $in; 
  @in = split(/&/,$in);
  ...
  foreach $i (0 .. $#in) {
    ...
    # Associate key and value : NOTE if you have "multiple" seperate with \0
    $in{$key} .= "\0" if (defined($in{$key})); # \0 is the multiple separator
    $in{$key} .= $val;
# Uncomment the following line for diagnostic 1
#    $in{'posting'} .= $post_string;
...
#by_number
#Sorts a list by numerics not string
sub by_number {
    $a <=> $b ;
}
# PrintHeader
...
sub PrintVariables {
  ...
  foreach $key (sort keys(%in)) {
    foreach (split("\0", $in{$key})) {  # \0 is the multiple seperator
      ($out = $_) =~ s/\n/<BR>/g;
...
#PrintVariablesShort
# Nicely formats variables in an associative array passed as a  parameter in alphabetical order.
# Using one line per pair (unless value is multiline)
# And returns the HTML string.
