...
static int OFIXBADCOL;  // 1 to apply bad column correction, 0 or not here = no
                        // bad column fix
static int OUSECORIMGS; // 1 for bias/thermal/flat corrections, 0 or not here =
                        // no corrections
...
void afoc_foc_cb(int code, char msg[]) {
  if (!afoc_ison())
    return;
...
void afoc_cam_cb(int code, char msg[]) {
...
#define NCCFG (sizeof(ccfg) / sizeof(ccfg[0]))
  static CfgEntry fcfg[] = {
      {"OFIRSTSTEP", CFG_DBL, &OFIRSTSTEP}, {"OSTOPSTEP", CFG_DBL, &OSTOPSTEP},
      {"OEXPTIM", CFG_DBL, &OEXPTIM},       {"OMINSTD", CFG_DBL, &OMINSTD},
...
static void mkAfGUI() {
  ...
  n++;
  XtSetArg(args[n], XmNtitle, "Auto Focus");
  n++;
  ...
  n++;
  XtSetArg(args[n], XmNrightPosition, 48);
  n++;
  ...
  n++;
  XtSetArg(args[n], XmNleftPosition, 52);
  n++;
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_FORM);
  n++;
  ...
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_POSITION);
  n++;
  XtSetArg(args[n], XmNrightPosition, 6);
  n++;
  ...
  XtSetArg(args[n], XmNrightAttachment, XmATTACH_POSITION);
  n++;
  XtSetArg(args[n], XmNrightPosition, 9);
  n++;
...
static void makeGap(Widget p_w) {
  ...
  XtVaSetValues(w, XmNseparatorType, XmNO_LINE, XmNheight, 10, NULL);
  XtManageChild(w);
}
...
static void makePrompt(Widget p_w, char *prompt, Widget *tfp,
  ...
  n++;
  XtSetArg(args[n], XmNcolumns, TFW);
  n++;
 ...
 */
static void makeLabelPair(Widget par_w, Widget *pp, Widget *lp) {
  Widget f_w, p_w, l_w;
  Arg args[20];
  int n;
  ...
  *pp = p_w;
  ...
  XtSetArg(args[n], XmNleftAttachment, XmATTACH_WIDGET);
  n++;
  XtSetArg(args[n], XmNleftWidget, p_w);
  n++;
  XtSetArg(args[n], XmNleftOffset, 10);
  n++;
  l_w = XmCreateLabel(f_w, "PL", args, n);
...
static void showGraph() {
  ...
  XtVaGetValues(sda_w, XmNwidth, &wid, XmNheight, &hei, NULL);
  wid -= 2 * BORD;
  hei -= 2 * BORD;
  ...
  for (i = 0; i < nimg; i++) {
    ...
    XDrawRectangle(dsp, win, graph_gc, x - BOXSZ / 2, y - BOXSZ / 2, BOXSZ,
                   BOXSZ);
    if (i > 0)
...
static void sdaCB(Widget w, XtPointer client, XtPointer call) { showGraph(); }
...
static void stopCB(Widget w, XtPointer client, XtPointer call) { doStop(); }
...
static void installCB(Widget w, XtPointer client, XtPointer call) {
  ...
  /* confirm */
  sprintf(buf, "Save and move to %.1fum @ %.1fC for filter %s", p, t,
          fip->name);
...
static int startExpose() {
  ...
  if (fifoMsg(Cam_Id, "Expose %d+%dx%dx%d %dx%d %g %d %d %s\n%s\n%s\n%s\n%s\n",
              0, 0, cep.sw, cep.sh, cep.bx, cep.by, dur, 1, 100, fn, obj,
              "Auto Focus via xobs", "Auto Focus", "Operator") < 0) {
...
static void camCB() {
  ...
  /* STO: 5/8/01  Apply corrections here during autofocus
          ...
          it's not entirely clear that this is what we really want to do
     always...
  */
  ...
  if (OFIXBADCOL) {
    if (!OBADCOLMAP[0]) {
      if (findMapFN(caldir, OBADCOLMAP, errmsg) < 0) {
        printf("Warning: Disabling Autofocus bad column corrections "
               "(OFIXBADCOL); %s\n",
               errmsg);
  ...
  if (OUSECORIMGS) {
    if (!OBIASIMG[0]) {
      if (findBiasFN(fip, caldir, OBIASIMG, errmsg) < 0) {
        printf("Warning: Disabling Autofocus Bias/Therm/Flat "
               "corrections(OUSECORIMGS); %s\n",
               errmsg);
...
static int nextMove(FImage *fip, double *movep) {
  ...
  if (aoi.sd < OMINSTD) {
    wlprintf(msg_w, "Stopping: StdDev = %.1f (must be >= %.1f)", aoi.sd,
             OMINSTD);
    return (-1);
  }
#endif
...
static double findNew() {
  ...
  bestnbr =
      imghist[besti - 1].sd > imghist[besti + 1].sd ? besti - 1 : besti + 1;
