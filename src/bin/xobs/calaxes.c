...
static void solveCB(Widget w, XtPointer client, XtPointer call) { solveNew(); }
...
static void installCB(Widget w, XtPointer client, XtPointer call) {
  ...
  if (installNew() == 0) {
    ...
    msg("Installed new calibration model: %g %g %g %g %g", tax.HT, tax.DT,
        tax.XP, tax.YC, tax.NP);
  }
}
...
static void showSolution(int empty) {
  ...
  wlprintf(new0_w, "  T Pole :     HA =%s  Dec = %s  NonP = %s", htbuf, dtbuf,
           npbuf);
...
static void solveNew() {
  ...
  for (i = 0; i < nobjs; i++) {
    ObjInfo *oip = &objs[i];
    printf("%10.7f %10.7f %10.7f %10.7f %s\n", oip->ha, oip->dec, oip->henc,
           oip->denc, oip->obj.o_name);
  }
  ...
  /* explain if failed */
  if (s < 0) {
    wlprintf(msg_w, "%d-star solution failed... see xobs.log for raw values.",
             nobjs);
    return;
  }
  ...
  /* suggest next step */
  if (nobjs < MAXOBJS)
    wlprintf(msg_w, "%d-star solution... add more, Undo, or Install if like.",
             nobjs);
  else
