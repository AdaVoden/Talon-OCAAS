...
/* name for each fifo */
typedef enum { H_AX, D_AX, R_AX, O_AX, I_AX, M_AX, N_AX } HLAxis;
typedef struct {
...
static HLInfo hl_info[N_AX] = {
    {"Scope HA/Az", Tel_Id, 'H'}, {"Scope Dec/Alt", Tel_Id, 'D'},
    {"Rotator", Tel_Id, 'R'},     {"Focus", Focus_Id},
    {"Filter", Filter_Id},        {"Dome", Dome_Id},
};
...
void g_focus(Widget w, XtPointer client, XtPointer call) { afoc_manage(); }

void g_calib(Widget w, XtPointer client, XtPointer call) { axes_manage(); }

void g_paddle(Widget w, XtPointer client, XtPointer call) { pad_manage(); }
...
static void homeCreateW() {
  ...
  XtVaSetValues(home_w, XmNverticalSpacing, 10, XmNhorizontalSpacing, 10,
  ...
  XtManageChild(rc_w);
  XtVaSetValues(rc_w, XmNtopAttachment, XmATTACH_FORM, XmNbottomAttachment,
                XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM,
                XmNrightAttachment, XmATTACH_FORM, XmNisAligned, True,
...
static void limitCreateW() {
  ...
  XtVaSetValues(limit_w, XmNverticalSpacing, 10, XmNhorizontalSpacing, 10,
                XmNautoUnmanage, False, NULL);
