...
static void mkPadGUI() {
  ...
  n++;
#if WINDSCREEN
  XtSetArg(args[n], XmNleftPosition, 65);
  n++;
#else
...
static Widget mkButtons(Widget p_w) {
  ...
  static ButtonW buttons[] = {
    ...
    {"E", "East wind screen", 50, &screenE_w, "East wind screen control"},
    {"F", "West wind screen", 60, &screenW_w, "West wind screen control"},
    {"G", "South wind screen", 70, &screenS_w, "South wind screen control"},
#elif COVER
...
static void buttonCB(Widget w, XtPointer client, XtPointer call) {
  ...
  if (w != oi_w)
    XmToggleButtonSetState(oi_w, False, True);
  if (w != coarse_w)
...
#if WINDSCREEN
  if (w != screenE_w)
    XmToggleButtonSetState(screenE_w, False, True);
  if (w != screenW_w)
    XmToggleButtonSetState(screenW_w, False, True);
  if (w != screenS_w)
    XmToggleButtonSetState(screenS_w, False, True);
#endif
#if COVER
  if (w != cover_w)
    XmToggleButtonSetState(cover_w, False, True);
#endif
