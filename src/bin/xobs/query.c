 ...
 */
void query(tw, msg, label0, label1, label2, func0, func1,
           func2) Widget tw; /* toplevel widget */
...
void rusure_seton(int whether) { rusure_silent = !whether; }

/* return true if dialog will be used, false if we just assume yes to all q's */
int rusure_geton() { return (!rusure_silent); }
