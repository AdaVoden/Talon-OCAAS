...
void updateStatus(int force) {
  ...
  busy = ds == DS_ROTATING || ds == DS_HOMING || ss == SH_OPENING ||
         ss == SH_CLOSING;
...
static void showSunMoon() {
  ...
  wtprintf(w, "%2.0f%% %2s %+.0f", op->s_phase, cardDirName(op->s_az),
           raddeg(op->s_alt));
  setColor(w, XmNbackground,
           op->s_alt > 0 ? ltcolors[LTACTIVE] : uneditableColor);
...
static void showWx() {
  ...
  if (time(NULL) - wp->updtime < 30) {
    ...
    setColor(w, XmNbackground,
             wp->alert & WXA_MAXW ? ltcolors[LTWARN] : uneditableColor);
    ...
    setColor(w, XmNbackground,
             wp->alert & WXA_MAXH ? ltcolors[LTWARN] : uneditableColor);

    w = g_w[IRAIN_W];
    wtprintf(w, "   %.1f mm", wp->rain / 10.);
    setColor(w, XmNbackground,
             wp->alert & WXA_RAIN ? ltcolors[LTWARN] : uneditableColor);

    w = g_w[ITEMP_W];
    wtprintf(w, "%8.1f C", np->n_temp);
    setColor(w, XmNbackground,
             wp->alert & (WXA_MINT | WXA_MAXT) ? ltcolors[LTWARN]
                                               : uneditableColor);
