...
focus_offset(int first, ...)
{
	...
	/* done when we reach goal */
        if((mip->haveenc && abs(mip->raw-rawgoal) < 2 && csi_rix(cfd,"=working;")==0 )
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: focus.c,v $ $Date: 2003/04/15 20:47:57 $ $Revision: 1.1.1.1 $ $Name:  $"};
