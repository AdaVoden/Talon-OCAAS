...
#define	MINAFDT	2.0	/* minimum autofocus temp change to cause a move */
...
focus_jog(int first, ...)
{
	...
	if (first) {
	    ...
	    case '0':	/* stop */
		stopFocus(0);
		fifoWrite (Focus_Id, 0, "Paddle command stop");
		active_func = NULL;
		return;
		...
		pc39_w ("a%c st wq jg%.0f",axis,mip->sign*MAXVELStp(mip)*OJOGF);
...
#define NHCFG   (sizeof(hcfg)/sizeof(hcfg[0]))
	...
	static int OHAVE, OHASLIM;
	static int OSTEP, OSIGN, OPOSSIDE, OHOMELOW;
	static double OLIMMARG, OMAXVEL, OMAXACC, OSLIMACC, OSCALE;
	static char OAXIS;

	static CfgEntry ocfg[] = {
	    {"OAXIS",		CFG_STR, &OAXIS, 1}, 
	    {"OHAVE",		CFG_INT, &OHAVE},
	...
	if (abs(OSIGN) != 1) {
	    fprintf (stderr, "OSIGN must be +-1\n");
	    die();
	}
	mip->sign = OSIGN;

	mip->limmarg = OLIMMARG;
...
stopFocus(int fast)
{
	int ac = IMOT->step * (fast ? IMOT->slimacc : IMOT->maxacc)/(2*PI);

	pc39_w ("a%c ac%d st", OMOT->axis, ac);
	telstatshmp->autofocus = 0;
	OMOT->homing = 0;
	OMOT->limiting = 0;
	OMOT->cvel = 0;
...
autoFocus()
{
	...
	pc39_w ("a%c l%c ma%d go", mip->axis, mip->havelim?'n':'f', rawgoal);
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: focus.c,v $ $Date: 2003/04/15 20:47:58 $ $Revision: 1.1.1.1 $ $Name:  $"};
