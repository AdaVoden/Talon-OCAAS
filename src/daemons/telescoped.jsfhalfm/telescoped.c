 ...
 * filter and focus commands and manipulates the lower-level hardware
 * accordingly. Several config files establish several options and parameters.
...
init_all()
{
	...
	/* initialize the global pc39 properties */
	if (pc39_w ("kl cn") < 0) {
	    fprintf (stderr, "pc39 command failed\n");
	    exit (1);
	}
...
periodic_update()
{
	/* update current time -- might be a dedicated process someday */
	telstatshmp->now.n_mjd = mjd_now();
}

/* For RCS Only -- Do Not Edit */
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: telescoped.c,v $ $Date: 2003/04/15 20:47:59 $ $Revision: 1.1.1.1 $ $Name:  $"};
