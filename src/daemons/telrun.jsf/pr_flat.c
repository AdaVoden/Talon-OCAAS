...
ps_wait4Setup(time_t n)
{
	...
	int allok = camok && telok && filok && domok;
	Scan *sp = cscan;
...
#define SPECIAL_FLAT_SHUTTER	-1
	...
	fifoWrite (Cam_Id,
			...
			sp->sx, sp->sy, sp->sw, sp->sh, sp->binx, sp->biny,
			    dur, /*CCDSO_Open*/SPECIAL_FLAT_SHUTTER, sp->priority, fullpath,
			sp->obj.o_name,
			sp->comment,
			sp->title,
			sp->observer);
...
//	int i;
	...
	/* build command line */
	/* FOR JSF: just send the NUMBER of flat frames, not their names
	l += sprintf (cmd+l, "calimagePV -F -l %c", cscan->filter);
	for (i = 0; i < nflat; i++) {
	    l += sprintf (cmd+l, " ");
	    l += tmpName (i, cmd+l);
	}
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: pr_flat.c,v $ $Date: 2003/04/15 20:48:01 $ $Revision: 1.1.1.1 $ $Name:  $"};
