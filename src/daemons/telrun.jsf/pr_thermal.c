...
//	int i;
	...
	/* build command line */
	/* FOR JSF: just send the NUMBER of thermal frames, not their names
	l += sprintf (cmd+l, "calimagePV -T");
	for (i = 0; i < ntherm; i++) {
	    l += sprintf (cmd+l, " ");
	    l += tmpName (i, cmd+l);
	}
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: pr_thermal.c,v $ $Date: 2003/04/15 20:48:01 $ $Revision: 1.1.1.1 $ $Name:  $"};
