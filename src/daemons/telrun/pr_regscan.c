...
pr_regSetup()
{
    ...
    if(sp->ccdcalib.new == CT_FIXEDALTAZ) {
        tlog(sp, "Moving to Alt:%.6f Az:%.6f",sp->extval1,sp->extval2);
        n = sprintf(buf,"Alt:%.6f Az:%.6f",sp->extval1,sp->extval2);
    } else {
    ...
    if(sp->ccdcalib.new == CT_FOCUSPOS) {
    ...
    } else if(sp->ccdcalib.new == CT_FOCUSOFF) {
...
ps_wait4Setup(time_t n)
{
    ...
    // We can't rely on this, 'cause the dome may be rotating to stay aligned with the tracking, so just check door
