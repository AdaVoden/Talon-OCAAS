...
URLUpdate (char *url, WxStats *wp, double *tp, double *pp)
{
	...
	while (recvlineb (fd, buf, sizeof(buf)) > 0) {
	    if ((sp = strstr (buf, WS_TPL)) != NULL && sp[-1] != '%') {
		wp->wspeed = atoi (sp+strlen(WS_TPL)+1);
		nfound++;
	    }
	    if ((sp = strstr (buf, WD_TPL)) != NULL && sp[-1] != '%') {
		wp->wdir = atoi (sp+strlen(WD_TPL)+1);
		nfound++;
	    }
	    if ((sp = strstr (buf, HM_TPL)) != NULL && sp[-1] != '%') {
		wp->humidity = atoi (sp+strlen(HM_TPL)+1);
		nfound++;
	    }
	    if ((sp = strstr (buf, RN_TPL)) != NULL && sp[-1] != '%') {
		wp->rain = 10*atof(sp+strlen(RN_TPL)+1);
		nfound++;
	    }
	    if ((sp = strstr (buf, PR_TPL)) != NULL && sp[-1] != '%') {
		*pp = 33.86*atof(sp+strlen(PR_TPL)+1);
		nfound++;
	    }
	    if ((sp = strstr (buf, TM_TPL)) != NULL && sp[-1] != '%') {
	...
	/* check for results */
	if (nfound != 6) {
	    daemonLog ("%s: did not find all keywords\n", url);
	    return (-1);
	}
	return (0);
}
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: url.c,v $ $Date: 2003/04/15 20:48:02 $ $Revision: 1.1.1.1 $ $Name:  $"};
