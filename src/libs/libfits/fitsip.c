 ...
 */
static int connected(CamPixel *peak, int w, int thresh) {
  static int lastw = -1;
  static int fan[8];
  int i, n;
...
static int ringAvg(CamPixel *peak) {
  static int lastw = -1;
  static int fan[8];
  int i, n;
...
// Assumed to be called when star finder gives us a qualified peak after doing
// bWalk.
...
static void blockWalk(CamPixel **pAddr, int dir, int dump) {
  ...
  if (oldW != bW_w) {
    ...
    for (i = 0; i < BLOCKSIZE; i++) {
      blockmap[i] = ((i / BLOCK_WH) - (BLOCK_WH / 2)) * bW_w +
                    ((i % BLOCK_WH) - (BLOCK_WH / 2));
    }
    oldW = bW_w;
    if (dump)
      printf("Made block map\n");
  }
...
int IsPointWithinStreakList(int x, int y, StreakData *streakList,
                            int nstreaks) {
  int i;
  int l, t, r, b;
  ...
  for (i = nstreaks;
       --i >= 0 /*&& streakList[i].endY >= t*/;) { // must read whole list!

    if (abs(streakList[i].startX - x) <= FSMINSEP &&
        abs(streakList[i].startY - y) <= FSMINSEP) {
      return 1; // start is already on list
    }
    if (streakList[i].length && streakList[i].endY >= t &&
        streakList[i].startY <= b) {
      int sl, sr;
...
/*
 * This is the entry point that is called by the star finder for finding
 * streaks. We assume that bWalk has been called prior to this and that the bW_
 * static variables contain the current peak and threshold data.
 *
 ...
 * We also go Left (direction 4), because a long low-slope streak may be too
 * shallow to trace with direction 3.
 *
 ...
 *
 * Return value is length of streak, or 0 if a star is found, or -1 if there was
 * an error or rejection Endpoints are returned via startx, starty, and endx,
 * endy (if not null)
 *
...
int walkStreak(int *startx, int *starty, int *endx, int *endy, int dump) {
  ...
  if (baseThresh < bW_thresh) {
    if (dump)
      printf("threshold rejection:%d < %d\n", baseThresh, bW_thresh);
    return -1;
  }
  ...
  if (dump)
    printf("tracing back to start\n");
  streakStartAddr = startAddr;
  blockWalk(&streakStartAddr, (longdir + 4) % 8, dump);

  if (dump)
    printf("***** {s} Seeded:%d,%d == Start: %4d, %4d  End: %4d, %4d  Length: "
           "%d\n",
           pixelX(startAddr), pixelY(startAddr), pixelX(streakStartAddr),
           pixelY(streakStartAddr), pixelX(endAddr[longdir]),
           pixelY(endAddr[longdir]), longest);

  if (startx)
    *startx = pixelX(streakStartAddr);
  if (starty)
    *starty = pixelY(streakStartAddr);
  if (endx)
    *endx = pixelX(endAddr[longdir]);
  if (endy)
    *endy = pixelY(endAddr[longdir]);
  return longest;
}
 ...
 */
static void slopeLength(int x, int y, int length, double slope, int *outX,
                        int *outY) {
  double m = fabs(slope);
  int dx, dy;
  if (m < 1.0) {
...
int dbon = 0;
static void widthWalk(CamPixel *addr, double slope, int thresh,
                      SEGINFO *pSegment) {
  ...
  while (1) {
    ...
    if (x1 + x < FSBORD || x1 + x > bW_w - FSBORD || y1 + y < FSBORD ||
        y1 + y > bW_h - FSBORD)
      break;
...
static int qualifyStreakData(CamPixel *im0, int w, StreakData *pStr) {
  ...
  */
  ...
  if (dbon)
    printf("Stats: min: %d  max: %d  mean: %lf, median: %lf, sd: %lf\n", min,
  ...
  for (i = 0; i < NSEG; i++) {
    slopeLength(pStr->walkStartX, pStr->walkStartY, i * seglen, pStr->slope, &x,
                &y);
  ...
  // -- use two metrics for 'wide' -- this will help qualify round objects
  // better
  rfloor = (pStr->length * 0.42); // 'round'
  wfloor = (pStr->length * 0.27); // 'wide'
  bfloor = median;
  if (dbon)
    printf("wfloor: %d  bfloor: %d  rfloor: %d\n", wfloor, bfloor, rfloor);
  wbits = bbits = rbits = 0;
  for (i = 0; i < NSEG; i++) {
    wbits <<= 1;
    bbits <<= 1;
    if (dbon)
      printf("%d) w: %d  mv: %d   b: %d\n", i, segInfo[i].width, midval[i],
             segInfo[i].bright);
    if (segInfo[i].width > rfloor)
      rbits++;
    if (segInfo[i].width > wfloor)
      wbits++;
    if (segInfo[i].width > 1) {
      if (midval[i] > bfloor)
        bbits++;
      else if (segInfo[i].bright > bfloor)
        bbits++;
    }
  }
...
#if NSEG == 5
    if (bbits == 0x11 || bbits == 0x13 || bbits == 0x1B || bbits == 0x19 ||
        bbits == 0x1D || bbits == 0x0D || bbits == 0x09
#elif NSEG == 4
    if (bbits == 0x05 || bbits == 0x09 || bbits == 0x0A || bbits == 0x0B ||
        bbits == 0x0D
#elif NSEG == 3
    if (bbits == 0x05
#endif
...
setFlags:
  qual = ((rbits << (NSEG * 3)) | (wbits << (NSEG * 2)) | (bbits << (NSEG)) |
          summary);
  if (dbon)
...
// call that will return streak data (which also will contain star data) in sa
// (if not null) and will also return old-style star data in xa,ya,ba (if not
// null). old style star count via return, streak count (which includes stars it
// found too) via the return pointer numStreaks.
int findStarsAndStreaks(char *im0, int w, int h, int **xa, int **ya,
  ...
  for (y = 0; y < nybox; y++) {
    ...
    for (x = 0; x < nxbox; x++) {
      ...
      if (fp)
        printf("T %5d %5d : %5d\n", x0 + boxw / 2, y0 + boxh / 2, boxes[boxi]);
    }
  }
  ...
  for (y = FSBORD; y < h - FSBORD; y++) {
    ...
    if (y == FSBORD) {
      ...
      for (x = FSBORD; x < w - FSBORD; x++)
        ybotrp[x] =
            linInterp(x0, boxw, boxes + nxbox * (yroll / boxh + 2), nxbox, x);
      ytop = y;
    }
    ...
    for (x = FSBORD; x < w - FSBORD; x++) {
      ...
      int thresh =
          ((double)(y)-ytop) * (ybotrp[x] - ytoprp[x]) / boxh + ytoprp[x];
      int brx, bry;
      CamPixel *peak;
      ...
      if (find_streaks) {
        ...
        // If it returns < 0 it means that it did NOT qualify anything for this
        // location
        int streakLength;
        ...
        // reject 0-length finds if they don't pass the star test for
        // connectivity
        ...
        if (streakLength >= 0) {
          ...
          pStr->slope = (pStr->walkEndX - pStr->walkStartX)
                            ? (double)(pStr->walkEndY - pStr->walkStartY) /
                                  (double)(pStr->walkEndX - pStr->walkStartX)
                            : HUGE_VAL;

          pStr->length = sqrt((pStr->walkEndX - pStr->walkStartX) *
                                  (pStr->walkEndX - pStr->walkStartX) +
                              (pStr->walkEndY - pStr->walkStartY) *
                                  (pStr->walkEndY - pStr->walkStartY));

          // Do another check to see if we end with a collision at end point
          if (IsPointWithinStreakList(pStr->endX, pStr->endY, streakList,
                                      nstreaks)) {
            if (dump)
              printf("endpoint already on streak list\n");
            if (std_findstars)
              goto starsearch; // note: not doing this would retain consistency
                               // between findstars and findstreaks
            else
              goto nope;
          }
          ...
          if (nstreaks % 100 == 0) {
            ...
            streakList = (StreakData *)realloc((void *)streakList,
                                               newCount * sizeof(StreakData));
          }
    ...
    starsearch:
      // continue with the rest of this if we are finding stars the old-school
      // way
      if (std_findstars) {
  ...
  if (find_streaks) {
    ...
    for (i = 0; i < nstreaks; i++) {
      if (!streakList[i].length) { // don't count the ones we have already
                                   // determined are streaks...
    ...
    //    printf("Looking for ratios < %lf or >
...
int findLinearFeature(char *im0, int w, int h, StarStats **ssp, double *xfirst,
                      double *yfirst, double *xlast, double *ylast) {
  ...
  for (i = 0; i < tcount; i++) {
    fr_std = sqrt((fr[i] - fr[i + 1]) * (fr[i] - fr[i + 1]));
    frstdev = 0.3;
    if (fr_std < frstdev) {
      nelemtemp++;
    } else {
...
void threeSort(int n, double *s, double *v1, double *v2) {
 ...
 */
void linFit(double *x, double *y, int ndata, double *a, double *b, double *siga,
 ...
 * stdev
 */
void getStats(double *x, int ndata, double *mean, double *median,
              double *stdev) {
  int i;
...
int removeOutliers(int ndata, double *x, double *y, double *fr) {
  int i, j;
  int count = 0;
 ...
 * N.B. we ignore pixels outside FSBORD.
 */
int findStatStars(im0, w, h, sspp) char *im0;
int w, h;
StarStats **sspp;
{
 ...
 */
static void starGauss(image, w, r, ssp) CamPixel *image; /* image array */
int w;                                                   /* width */
 ...
 */
static void brightSquare(image, w, ix, iy, r, xp, yp,
                         bp) CamPixel *image; /* image */
 ...
 */
static int brightWalk(imp, w, x0, y0, maxr, xp, yp, bp) CamPixel *imp;
int w;
int x0, y0;
int maxr;
int *xp, *yp;
 ...
 * Based on Larry Molnar notes of 6 Dec 1996
 */
static void bestRadius(image, w, x0, y0, rAp,
                       rp) CamPixel *image; /* array of pixels */
int w;                                      /* width of image */
 ...
 * about [x0,y0]
 */
static void ringCount(image, w, x0, y0, r, np,
                      sump) CamPixel *image; /* array of pixels */
int w;                                       /* width of image */
int x0, y0;                                  /* center of annulus */
...
static void ringStats(image, w, x0, y0, r, Ep,
                      sigp) CamPixel *image; /* array of pixels */
int w;                                       /* width of image */
int x0, y0;                                  /* center of annulus */
 ...
 */
static int skyStats(image, w, h, x0, y0, r, Ep,
                    sigp) CamPixel *image; /* array of pixels */
int w;                                     /* width of image */
 ...
 */
static void circleCount(image, w, x0, y0, maxr, np,
                        sump) CamPixel *image; /* array of pixels */
int w;                                         /* width of image */
int x0, y0;                                    /* center of annulus */
...
char *getCurrentIpCfgPath(void) { return ipcfn; }

void setIpCfgPath(char *pathname) { strncpy(ipcfn, pathname, sizeof(ipcfn)); }

/* For CVS Only -- Do Not Edit */
static char *rcsid[2] = {
    (char *)rcsid,
    "@(#) $Id: fitsip.c,v 1.1.1.1 2003/04/15 20:48:15 lostairman Exp $"};
