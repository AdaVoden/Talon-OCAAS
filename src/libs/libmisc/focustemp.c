...
/************************************************************\
	...
	CVS History:
		$Id: focustemp.c,v 1.1.1.1 2003/04/15 20:48:18 lostairman Exp $
		$Log: focustemp.c,v $
		Revision 1.1.1.1  2003/04/15 20:48:18  lostairman
		Initial Import
...
//		TRACE "finding brackets for %.2lf / %.2lf\n",lt,ht);
		...
		if(!lowfound) {
			...
			plt = &focusPositionTable[0];
			// the highfound count will be the next "bracket" high value we should use in this case
			if(highfound > focusPositionEntries-1) highfound = focusPositionEntries-1; // for safety
			pht = &focusPositionTable[highfound];				
		}
		if(!highfound) {
			int hi;
			
			pht = &focusPositionTable[focusPositionEntries-1]; // use highest value we have
