...
extern int
makefits(outfile,nx,ny,bitpix,datatype)
FILE *outfile;
int nx,ny;
int bitpix;
char *datatype;
...
int i;
	...
	sprintf(&line[10], "'%*.*s'", strlen(datatype),strlen(datatype),datatype);
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: makefits.c,v $ $Date: 2003/04/15 20:48:34 $ $Revision: 1.1.1.1 $ $Name:  $"};
