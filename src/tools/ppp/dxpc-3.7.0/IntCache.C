...
IntCache::IntCache(unsigned int size):
  ...
  lastValueInserted_(0), lastDiff_(0), predictedBlockSize_(0)
{
}


int
IntCache::lookup(unsigned int &value, unsigned int &index, unsigned int mask,
...
unsigned int
IntCache::get(unsigned int index)
{
...
void
IntCache::insert(unsigned int &value, unsigned int mask)
{
  unsigned int insertionPoint;
  if (2 >= length_)
    insertionPoint = length_;
