...
ServerChannel::ServerChannel(int xServerFD, unsigned int statisticsLevel):
  readBuffer_(xServerFD, this),
  fd_(xServerFD),
  firstRequest_(1),
  firstReply_(1),
...
int
ServerChannel::doRead(EncodeBuffer& encodeBuffer)
{
	  ...
	  case X_AllocNamedColor:
	    {
	      ...
	      {
		encodeBuffer.encodeValue(GetULONG(nextSrc, bigEndian_), 32,
					 9);
		nextSrc += 4;
	      }
	...
	case LeaveNotify:
	  {
		...
		encodeBuffer.encodeCachedValue(GetULONG(nextSrc, bigEndian_),
		       29, *serverCache_.motionNotifyWindowCache[i], 6);
		nextSrc += 4;
	      }
	...
	case SelectionRequest:
	  {
	    ...
	    encodeBuffer.encodeCachedValue(GetULONG(buffer + 20, bigEndian_),
			   29, serverCache_.selectionClearAtomCache, 9);
	    encodeBuffer.encodeCachedValue(GetULONG(buffer + 24, bigEndian_),
			   29, serverCache_.selectionClearAtomCache, 9);
	  }
	  break;
...
int
ServerChannel::doWrite(const unsigned char *message, unsigned int length)
{
  writeBuffer_.reset();
...
void
ServerChannel::encodeCharInfo_(const unsigned char *nextSrc,
