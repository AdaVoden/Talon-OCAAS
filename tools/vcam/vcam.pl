...
# get parameters
$TELHOME = $ENV{TELHOME};
do "$TELHOME/archive/config/vcam.cfg";
...
sub getNetImage
{
	...
	if($shtr > 0) {
        ...
        my $lastRqst = `cat $TELHOME/archive/lastRqst`;
        chomp($lastRqst);
        if($lastRqst eq $rqst) {
        	&trace("Repeating image");
        	`cp $TELHOME/archive/lastimg $sfim`;
        	return;
        }
        &trace("$lastRqst != $rqst");
        `echo "$rqst" > $TELHOME/archive/lastRqst`;
...
#    		               $url,$survey,$rah,$ram,$ras,$dcd,$dcm,$dcs,$vminrq,$hminrq);
		...
		# download the image
		`lynx -dump "$rqst" > $sfim`;
	    `cp $sfim $TELHOME/archive/lastimg`;
