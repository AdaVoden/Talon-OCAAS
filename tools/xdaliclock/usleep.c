     ...
     unsigned long usecs;
{
  ...
  extern int SYS$SCHDWK (), SYS$HIBER (); 
  
  if (!deltas_set) set_deltas ();
  bin_delta = (int *)((usecs == TICK_INTERVAL) ? &bin_tick_delta :
                                               &bin_sec_delta);
...
static char *rcsid[2] = {(char *)rcsid, "@(#) $RCSfile: usleep.c,v $ $Date: 2003/04/15 20:48:48 $ $Revision: 1.1.1.1 $ $Name:  $"};
